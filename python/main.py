...
if __name__ == '__main__':
    ...
    predictor = Predictor(model_file, 224)

    input_dir = os.getenv('INPUT_DIR')
    def run(mode: str):
        for filename in os.listdir(os.path.join(input_dir, mode)):
            input_file = os.path.join(input_dir, mode, filename)
            label, probs = predictor.predict(input_file)
            print(probs[0])
            print(label)
